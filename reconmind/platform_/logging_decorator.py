    ...
    def decorator(fn: NodeFn) -> NodeFn:
        ...
        def wrapper(state: GraphState) -> GraphState:
            ...
            _write_event(
                ...
                latency_ms=latency_ms,
                timestamp=timestamp,
                defense_active=1 if result.get("defense_active") else 0,
                defense_triggered=1 if result.get("defense_triggered") else 0,
                defense_confidence_score=result.get("defense_confidence_score"),
                temperature=temperature,
                input_tokens=input_tokens,
                output_tokens=output_tokens,
            )
